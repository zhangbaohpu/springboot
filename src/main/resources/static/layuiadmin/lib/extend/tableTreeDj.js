    ...
    class Tree {
        constructor() {
            this.config = {
                ...
                // title字段使用 `templet` 函数, 此函数会导致数据出现问题,目前无法解决.
                // 因此将图标与缩进去掉.并且组件提供另外的方法(setTitleIcon, setTitleIndent)以使其可以在组件内部调用
                ,titleUseTemplet: false
            };
            // 运行数据模板
            this.runTemplate = {
        ...
        // 设置标题字段的图标
        setTitleIcon(id, title) {
            const hasChild = this.getHasChild();
            const iconClose = this.getIconClose();
            if(hasChild.indexOf(id) !== -1) {
                title = '<span class="'+ iconClose +'"></span>' + title;
            }
        ...
        _getTitleUseTemplet() {
            return this.config.titleUseTemplet;
        }
